...
exports.createProperty = async (propertyData, userId, transaction) => {
    if (!propertyData.address || propertyData.address.length < 5) {
        throw new Error("Property address is required and should be at least 5 characters."); 
    }
    // Validate user role
    ...
    const existingProperty = await property.findOne({ where: { address: propertyData.address } });
    if (existingProperty) {
